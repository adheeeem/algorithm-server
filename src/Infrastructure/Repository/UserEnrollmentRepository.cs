...
public class UserEnrollmentRepository(IDbConnection connection) : IUserEnrollmentRepository
{
	...
	{
		const string query = $"select 1 from {UserEnrollmentTable} where app_user_id=@userId and unit_number=@unitNumber and paid=TRUE";
	...
	{
		const string query = $"insert into {UserEnrollmentTable} (app_user_id, unit_number, paid) values (@userId, @unitNumber, @isPaid) returning id";
	...
	{
		const string query = $"update {UserEnrollmentTable} set enrolled=TRUE where unit_number=@unitNumber and app_user_id=@userId;";
	...
	{
		const string query = $"select enrolled, paid, unit_number as unitNumber, is_completed as isCompleted, date from {UserEnrollmentTable} where app_user_id=@userId and unit_number=@unitNumber";
	...
	{
		const string query = $"update {UserEnrollmentTable} set paid=@isPaid where unit_number=@unitNumber and app_user_id=@userId;";
